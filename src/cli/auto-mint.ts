...
import { createCustomAcaiPsbt, customAcaiExecute } from '../alkanes/alkanes.js';
import { Wallet } from './wallet.js';
import { encodeProtostone } from '../alkanes/alkanes.js';
...
interface AutoMintConfig {
  ...
  provider: string;
  feeRate: number;
  maxBlockTransactions: number;    // 最大区块交易数量
  maxFeeRate: number;             // 最大费率
  waitMinutes: number;            // 新区块出现后等待的分钟数
  mintCount: number;              // 每次mint的数量
}
...
class AutoMintManager {
  ...
  private currentMintTxs: string[] = []; // 改为数组存储多个交易
  private running: boolean = false;
  private pendingBlocks: Map<number, { firstScanTime: number, blockInfo: BlockInfo }> = new Map();
   ...
   * 获取最新区块信息
   */
  private async getLatestBlockInfo(): Promise<BlockInfo> {
    try {
      const response = await axios.get('https://mempool.space/api/blocks/tip/height');
      const latestHeight = response.data;
      
      const blockResponse = await axios.get(`https://mempool.space/api/v1/blocks/${latestHeight}`);
   ...
   * 获取指定区块的信息
   */
  private async getBlockInfo(blockHeight: number): Promise<BlockInfo> {
    try {
      const blockResponse = await axios.get(`https://mempool.space/api/v1/blocks/${blockHeight}`);
      const blockData = blockResponse.data[0];
  ...
  private async getTransactionStatus(txid: string): Promise<TransactionStatus> {
    try {
      const response = await axios.get(`https://mempool.space/api/tx/${txid}/status`);
      return {
   ...
   */
  private isBlockSuitableForMint(blockInfo: BlockInfo): boolean {
    console.log(`📊 区块分析:`);
   ...
   * 执行ACAI mint（批量）
   */
  private async executeMint(): Promise<string[]> {
    ...
    try {
      ...
      // 创建钱包实例
      const wallet = new Wallet({ networkType: this.config.provider as any });
      
      // 编码protostone数据
      const protostoneData = encodeProtostone({
      ...
      for (let i = 0; i < this.config.mintCount; i++) {
        try {
          ...
          const result = await customAcaiExecute({
   ...
   * 立即寻找下一个可mint的区块
   */
  private async findNextMintableBlock(): Promise<void> {
    try {
   ...
   * 主监控循环
   */
  public async startAutoMint(): Promise<void> {
    this.running = true;
