...
const send = async ({ gatheredUtxos, toAddress, amount, runeId, inscriptionAddress, feeRate, account, provider, signer, noBroadcast, }) => {
    ...
    if (noBroadcast) {
        ...
        return {
            ...
            message: '交易已签名但未广播。你可以使用 rawTx 手动广播交易。'
        };
    }
...
exports.send = send;
const mint = async ({ gatheredUtxos, account, runeId, provider, feeRate, signer, noBroadcast, }) => {
